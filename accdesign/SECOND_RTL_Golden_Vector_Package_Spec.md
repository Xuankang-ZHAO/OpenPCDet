...
- 固定 KITTI 帧只在 `manifest.json` 中以 split、sample ID、原始 `.bin` 的
  SHA-256 标识；**不复制**原始点云。
- 保存第一层的量化 VFE 输入；这是 RTL 唯一外部 IFM 输入。
...
- `format_version`、`word_bits: 64`、`byte_order: "little"`；
...
- `partition_geometry`：当前 Block Manager 使用的 partition geometry；
- `input_pages`：初始 VFE 输入的 block 顺序。每个 block 给出 `block_id`、
  `total_voxel_count`；每个 page 给出 `page_addr`、`voxel_count`、
  `word_offset`。一个 page 的 `voxel_count` 必须为 `1..64`，同一 block 的 page
  计数和必须等于 `total_voxel_count`；
...
- `rtl_contract`：`cin_tile=8`、`cout_tile=8`、`bias_bits=16`、`shift_bits=4`、
  `weight_kernel_order="x*9+y*3+z"`，以及本规范第 6 节列出的 layer-11 状态。
...
### 3.2 `vfe_input_pages.bin`（唯一外部 IFM）

它是送入 layer 0（`conv_input.0`）的量化 VFE 稀疏输入，按 manifest 指定的
`block -> page -> voxel` 顺序顺序拼接。每个 voxel 恰有两个 64-bit word：
...
- 坐标必须在硬件可表达范围内；每个 page 内不得有重复 `coord32`。输入 page 的
  `is_halo` 由当前 Block Manager / partition 流程确定，不能在导出器中随意排序或
  清零。

该文件采用当前 `memory_block_load_dma` 的 voxel-major DRAM 格式。保存的坐标是
全局坐标；`block_coord_adapter` 在硬件中负责转换为 local coordinate。因此不能只
导出算法侧的 `[batch,z,y,x]` tensor，而忽略 block/page 描述符。
...
5. 检查每个 input page 的 voxel 数为 `1..64`、block 中 page count 总和正确、坐标
   可编码且无重复。
...
1. 将 `weights.bin` 和 `params.bin` 依据 manifest 加载至层 DMA 的相应地址；将
   `vfe_input_pages.bin` 及 manifest 中的 block/page descriptor 输入 Block Manager
   / Memory 侧。
2. 让硬件连续执行 layer 0 至 layer 11。层间 OFM 由硬件自身写回并再次读取；测试
   平台不得把 `ofm_golden.bin` 回灌为下一层输入。
3. 每层结束后，将 RTL 产生的 OFM 解码为 `{coord32, INT8[Cout]}`，按 `coord32`
   升序排序，并与该层 `ofm_golden` 分段做完全相等比较。
4. 通过标准是：点数相等、坐标集合相等、每一个 INT8 byte 相等；没有 epsilon、
