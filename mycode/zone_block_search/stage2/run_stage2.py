#!/usr/bin/env python3
"""Stage-2 zone / block-size search: Steps 0–3 then pause.

Profiles the Bx>By menu over 200 KITTI FOV frames mapped to conv3 occupancy,
...
from mycode.zone_block_search.stage2.partition import (
...
def plot_results(out_dir: Path, rings: list, menu, coverage_table: dict, bstar_rows: list,
    ...
    ax.set_title(f'Stage2 two-page coverage R (q={q}, limit={TWO_PAGE_LIMIT})')
    ...
    ax.set_ylabel('B*(j)')
    ax.set_title('Stage2 preferred size per fine ring (red = low_sample)')
...
def main():
    parser = argparse.ArgumentParser(description='Stage2 zone/block search Steps 0–3')
