#!/usr/bin/env python3
"""Per-frame whole-scene two-page coverage under a Stage-1 zone LUT.

Uses rtl_unfixed multi-zone partition (variable Bx×By×Bz + 3D halo) on conv2
...
def write_lut(path: Path, lines: list):
    ...
    with path.open('w') as handle:
        handle.write('# Auto-generated Stage1 zone LUT (Bz=8)\n')
...
def main():
    parser = argparse.ArgumentParser(description='Stage1 per-frame R under zone LUT')
    ...
    try:
        ...
        ax.set_title('Stage1 whole-scene R (7 RLE zones)')
        ax.legend()
        fig.tight_layout()
    ...
    print('\n=== Per-frame whole-scene R (7 RLE zones) ===')
