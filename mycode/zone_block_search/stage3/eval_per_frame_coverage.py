#!/usr/bin/env python3
"""Per-frame whole-scene two-page coverage under a Stage-3 zone LUT.

Uses rtl_unfixed multi-zone partition (variable Bx×By×Bz + 3D halo) on conv4
...
def write_lut(path: Path, lines: list):
    ...
    with path.open('w') as handle:
        handle.write('# Auto-generated Stage3 zone LUT (Bz=8)\n')
...
def main():
    parser = argparse.ArgumentParser(description='Stage3 per-frame R under zone LUT')
    ...
    try:
        ...
        ax.set_title('Stage3 whole-scene R')
        ax.legend()
        fig.tight_layout()
