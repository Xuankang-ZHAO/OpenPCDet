#!/usr/bin/env python3
"""Stage-3 zone / block-size search: Steps 0–3 then pause.

Profiles the Bx>By menu over 200 KITTI FOV frames mapped to conv4 occupancy,
...
from mycode.zone_block_search.stage3.partition import (
    accumulate_ring_nb_samples,
    summarize_ring_coverage,
    validate_against_rtl_unfixed,
)
from mycode.zone_block_search.stage3.voxels import load_stage3_voxel_frames
...
def plot_results(out_dir: Path, rings: list, menu, coverage_table: dict, bstar_rows: list,
    ...
    ax.set_title(f'Stage3 two-page coverage R (q={q}, limit={TWO_PAGE_LIMIT})')
    ...
    ax.set_ylabel('B*(j)')
    ax.set_title('Stage3 preferred size per fine ring (red = low_sample)')
...
def main():
    parser = argparse.ArgumentParser(description='Stage3 zone/block search Steps 0–3')
    ...
    frame_ids, coords_list, meta = load_stage3_voxel_frames(
