#!/usr/bin/env python3
"""INT8 SECOND 3D-backbone occupancy under a single fixed 10x10x6 block.

Same consumer window-corner halo and DRAM page allocator as
`proposed page allocation`, but every stage/layer tiles from grid origin
(0, 0, 0) with one XYZ block size. No Chebyshev distance and no zone LUT.
Block indices follow `mycode/rtl_fixed`: `coord // block_size`.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Fixed 10x10x6 INT8 SECOND 3D-backbone IFM/OFM DRAM footprint'
    )
...
def build_layer_rows(
    ...
    for index, tensor in enumerate(tensors):
        ...
        tensor_stats.append(
            analyze_tensor(tensor, consumer, bin_width, block_size_xyz, allocate_pages=allocate)
        )
...
def build_markdown(payload: dict) -> str:
    ...
    lines = [
        ...
        '',
        f"- L11 OFM pages: `{summary.get('last_ofm_pages')}` (must be 0)",
        f"- Peak at `{summary.get('peak_layer_name')}`: `{summary.get('peak_pages')}` pages / `{summary.get('peak_bytes')}` Byte",
        '',
        markdown_table(
            ['State', 'Pages', 'Sum N_b', 'Nonempty blocks', 'Grid tiles'],
            [
                [item['state'], item['pages'], item['sum_nb'], item['nonempty_blocks'], item['grid_tiles']]
