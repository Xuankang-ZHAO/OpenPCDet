#!/usr/bin/env python3
"""Compare three DRAM schemes on KITTI train 000000-000019.

Reuses the 000216 analysis path: FOV KITTI loading, hardware-reference INT8
SECOND 3D backbone, consumer window-corner halo. Writes one markdown with
per-frame Feature Map tables and IFM+OFM coresident peaks.
"""
...
def parse_args():
    ...
    parser.add_argument('--id_end', type=int, default=19)
    parser.add_argument('--device', type=str, default='auto')
    parser.add_argument('--weight_quant', choices=['per_channel', 'per_tensor'], default='per_channel')
    parser.add_argument(
        '--out_md',
        type=str,
        default=str(_SCRIPT_DIR / 'train_000000_000019_feature_map_dram_hash_entry_comparison.md'),
    )
    parser.add_argument(
        '--out_json',
        type=str,
        default=str(_SCRIPT_DIR / 'train_000000_000019_feature_map_dram_hash_entry_comparison.json'),
    )
...
def attach_trainval_infos(dataset, project_root: Path) -> Dict[str, int]:
    infos = []
    seen = set()
...
def build_markdown(payload: dict) -> str:
    ...
    for frame in frames:
        ...
        lines.append(
            f"| `{frame['frame_id']}` | {frame['coordinate_sha256']['input']} | {hit} | "
...
def main():
    ...
    info_by_id = attach_trainval_infos(dataset, project_root)
    ...
    model.to(device)
    model.eval()
    hw_dir = _SCRIPT_DIR / 'train_000000_000019_hw_export'
    ...
    for frame_id in frame_ids:
        print(f'Analyzing train/{frame_id} ...', flush=True)
